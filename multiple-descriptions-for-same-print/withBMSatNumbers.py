#Make file of all LWL records with BM Sat numbers, giving LWL record number, BMSat number, description
import re
import html
...
o.write('LWL number\t500 Match\tBMSat from 500\t510 Match\tBMSat from 510\tBMSat (from 500, else 510)\tBM online\tDescription (from 520)\n')
linesPerBMSatNumber = defaultdict(list)
for r in recs:
    s = re.search('\d+', ' '.join(catNumber500[r]))
    if s:
        n500 = s.group(0)
    else: n500 = 0

    s = re.search('\d{3,}', ' '.join(catNumber510[r]))
    if s:
        n510 = s.group(0)
    else: n510 = 0
...
o1.write('LWL number\t500 Match\tBMSat from 500\t510 Match\tBMSat from 510\tBMSat (from 500, else 510)\tBM online\tDescription (from 520)\n')
